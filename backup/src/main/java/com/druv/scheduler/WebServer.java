...
public class WebServer {
    ...
    */
    ...
    @PostMapping("/api/timetable/delete-by-slot")
    @ResponseBody
    public ResponseEntity<Map<String, Object>> deleteTimetableBySlot(@RequestBody Map<String, String> payload) {
        try {
            String dayOfWeek = payload.get("dayOfWeek");
            String startTime = payload.get("startTime");
            String endTime = payload.get("endTime");
            
            boolean success = timetableDAO.deleteBySlot(dayOfWeek, startTime, endTime);
            if (success) {
                return ResponseEntity.ok(Map.of("success", true, "message", "Entry deleted successfully"));
            } else {
                return ResponseEntity.status(HttpStatus.NOT_FOUND)
                    .body(Map.of("success", false, "message", "Entry not found"));
            }
        } catch (Exception e) {
            return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR)
                .body(Map.of("success", false, "message", "Error deleting entry: " + e.getMessage()));
        }
    }
